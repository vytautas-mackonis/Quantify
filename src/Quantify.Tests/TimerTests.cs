...
namespace Quantify.Tests
{
    public class TimerTests
    {
        ...
        public void InitialTimerHasZeroValues(int[] movingRateDurations, double[] percentiles)
        {
            ...
            var expectedPercentiles = percentiles.Select(x => new PercentileValue<long>((decimal)x, 0)).ToArray();
        ...
        public void TimingWillUpdateLatencies(int[] timings, double[] percentiles)
        {
            ...
            var expectedPercentiles = percentiles.Select(x => new PercentileValue<long>((decimal)x, Statistics.Percentile(timings, x))).ToArray();
            Assert.Equal(expectedPercentiles, latencies.Percentiles);
        }
